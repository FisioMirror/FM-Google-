...
export function Login() {
  ...
  const fillFisioDemo = async () => {
    switchRole('fisioterapeuta');
    setMode('login');
    setEmail('fisio@demo.com');
    setPassword('demo1234');
    toast.info('Iniciando sesión demo de Fisioterapeuta...');
    const ok = await signIn('fisio@demo.com', 'demo1234');
    if (ok) {
      toast.success('Bienvenido al Modo Demo de Fisioterapeuta');
    }
  };

  const fillPacienteDemo = async () => {
    switchRole('paciente');
    setMode('login');
    setToken('123456');
    toast.info('Iniciando sesión demo de Paciente...');
    const ok = await handleTokenSubmit('123456');
    if (ok) {
      toast.success('Bienvenido al Modo Demo de Paciente');
    }
  };
