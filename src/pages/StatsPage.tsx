...
export function StatsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const demoPw: PatientWithSessions[] = UNIFIED_DEMO_PATIENTS.map(p => ({
      id: p.id,
      full_name: p.name,
      diagnostico: p.diagnosis,
      sessionCount: p.sessionsCompleted,
      avgQuality: p.progress,
      lastSessionDate: p.sessions[0]?.fecha || '2026-03-25T10:00:00Z',
    }));

    const demoAllSessions: SessionRow[] = UNIFIED_DEMO_PATIENTS.flatMap(p =>
      p.sessions.map(s => ({
        paciente_id: p.id,
        fecha: s.fecha,
        ...
        calidad_ejecucion: s.calidad_ejecucion,
      }))
    );
    ...
    try {
      if (isFisio) {
        ...
        const { data: allSessions } = await supabase
          ...
          .order('fecha', { ascending: false });
        const s = (allSessions && allSessions.length > 0 ? allSessions : PRIMARY_DEMO_PATIENT.sessions) as unknown as SessionRow[];
        setSessions(s);
        setTotalSessions(s.length);
        setTotalMinutes(Math.round(s.reduce((sum, r) => sum + (r.duracion_segundos || 0), 0) / 60));
        setAvgPrecision(s.length > 0 ? Math.round(s.reduce((sum, r) => sum + (r.calidad_ejecucion || 0), 0) / s.length) : 0);
