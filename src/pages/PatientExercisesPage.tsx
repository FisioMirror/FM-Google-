...
export function PatientExercisesPage() {
  ...
  const loadExercises = async () => {
    ...
    try {
      ...
      if (normalized.length === 0 && (user.id?.startsWith('demo') || user.email === 'paciente@demo.com')) {
        const demoExercises: AssignedExercise[] = [
          {
            id: 'demo-ex-1',
            ejercicio_nombre: 'Flexión de hombro',
            ejercicio_id: 'flexion-hombro',
            ejercicio_detailed_description: 'Comienza de pie con los brazos a los lados. Eleva lentamente el brazo hacia adelante hasta alcanzar los 90° (paralelo al suelo). Mantén la posición 3 segundos y baja con control.',
            series: 3,
            repeticiones: 12,
            frecuencia_semana: 4,
            notas: 'Mantener el torso recto y la escápula estable.',
            activo: true,
          ...
          },
          {
            ...
            ejercicio_detailed_description: 'Codo flexionado a 90° pegado al costado. Rota el antebrazo hacia afuera manteniendo el eje del codo.',
            series: 3,
            repeticiones: 15,
            frecuencia_semana: 4,
            notas: 'No despegar el codo del torso.',
            activo: true,
            fecha_asignacion: new Date().toISOString(),
          },
          {
            ...
            ejercicio_detailed_description: 'Realiza círculos controlados con los hombros hacia atrás y hacia adelante.',
            series: 2,
            repeticiones: 10,
            frecuencia_semana: 3,
            notas: 'Movimiento lento sin forzar rangos dolorosos.',
            activo: true,
            fecha_asignacion: new Date().toISOString(),
          },
          {
            ...
            ejercicio_detailed_description: 'Inclina suavemente la cabeza hacia el lado opuesto y mantén la tensión durante 20 segundos.',
            series: 2,
            repeticiones: 4,
            frecuencia_semana: 5,
            notas: 'Respiración diafragmática continua.',
            activo: true,
            fecha_asignacion: new Date().toISOString(),
          },
        ];
        ...
        setIsOffline(false);
        saveExercises(demoExercises as unknown as Record<string, unknown>[]);
        return;
      }
