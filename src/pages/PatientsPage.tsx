...
export function PatientsPage() {
  ...
  const loadPatients = async () => {
    setLoading(true);
    const demoCards: PatientCard[] = UNIFIED_DEMO_PATIENTS.map((p, i) => ({
      id: p.id,
      name: p.name,
      fmId: p.fmId,
      status: p.status === 'Alta próxima' ? 'Alta próxima' : p.status === 'Requiere Revisión' ? 'Requiere Revisión' : p.progress >= 75 ? 'Mejorando' : 'Estable',
    ...
    try {
      ...
      // Merge real cards with demo cards avoiding duplicate names
